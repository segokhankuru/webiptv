...
const pool = connectionString ? new Pool({
    ...
    // Serverless ortam için optimize
    max: 5,
...
class PreparedStatement {
    constructor(sql) {
        ...
        // PostgreSQL LIMIT ve OFFSET parametrelerinin integer olmasını bekler.
        // pg kütüphanesi parametreleri text olarak gönderir, bu yüzden ::int cast gerekli.
        this.sql = this.sql.replace(/LIMIT\s+(\$\d+)/gi, 'LIMIT $1::int');
        this.sql = this.sql.replace(/OFFSET\s+(\$\d+)/gi, 'OFFSET $1::int');
    }
...
// Migration SQL'leri — her biri ayrı statement olarak çalıştırılır
...
// initDb — her migration statement'ı ayrı ayrı çalıştırır
...
export const initDb = () => {
    ...
    _initPromise = (async () => {
        if (!pool) {
            console.error("initDb skipped: no database connection string.");
            return;
        }
        try {
...
// İlk yükleme anında migration'ı başlat
initDb();
